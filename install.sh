...
printf "${bgreen}recon-bluster installer script ${reset}\n\n"

install_apt(){
    eval $SUDO apt update && $SUDO apt install -y curl sqlmap jq python3-pip
}

install_yum(){
    eval $SUDO yum install -y curl sqlmap jq python3-pip
}

install_pacman(){
    eval $SUDO pacman -Sy install -y curl sqlmap jq python3-pip
}
...
printf "${bgreen}System paccatkages installed${reset}\n\n"
...
printf "${bgreen}Finished! Please reboot your system.${reset}\n\n"